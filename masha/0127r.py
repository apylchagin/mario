...
class Tree:
    def __init__(self, rect, fr0, fr1, fr2):
        ...
        self._num = 0
        self._rect = rect
        self._lastUpdate = self.__getTime()
    def draw(self, surface):
        if (self.__getTime() - self._lastUpdate) >= 200:
            self._num = (self._num + 1) % 3
            self._lastUpdate = self.__getTime()
        if self._num == 0:
            surface.blit(self._fr0, self._rect)
        elif self._num == 1:
            surface.blit(self._fr1, self._rect)
        else:
            surface.blit(self._fr2, self._rect)
    def __getTime(self):
...
tree1 = Tree(
    pygame.Rect(0, H - GROUND_H - 321, 30, H - GROUND_H),
    pygame.image.load('1Tree.png'),
    pygame.image.load('2Tree.png'),
    pygame.image.load('3Tree.png'))
tree2 = Tree(
    pygame.Rect(300, H - GROUND_H - 321, 330, H - GROUND_H),
    pygame.image.load('2Tree.png'),
    pygame.image.load('3Tree.png'),
    pygame.image.load('1Tree.png'))
